<template>
  <div class="match">
    <div class="match-head">
      <span class="badge">Spiel {{ number }} · {{ String(match.mode).replace('v',' gegen ') }}</span>
      <button v-if="editable" class="btn danger" @click="$emit('delete', match.id)">Löschen</button>
    </div>
    <div class="teams">
      <div class="team"><b>Team A</b><div v-for="id in match.team_a" :key="id">{{ nameOf(id) }}</div></div>
      <div class="team"><b>Team B</b><div v-for="id in match.team_b" :key="id">{{ nameOf(id) }}</div></div>
    </div>
    <div class="score">
