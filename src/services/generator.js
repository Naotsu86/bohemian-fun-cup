...
function strength(p) {
  return Number(p.strength || 1) + Number(p.form || 0)
}
...
function teamSum(team) {
  return team.reduce((sum, p) => sum + strength(p), 0)
}

function teamPenalty(team, h) {
  let p = 0
  for (let i = 0; i < team.length; i++) {
    for (let j = i + 1; j < team.length; j++) {
      p += h.together.get(pairKey(team[i].id, team[j].id)) || 0
    }
  }
  return p
}

function opponentPenalty(a, b, h) {
  let p = 0
  for (const x of a) for (const y of b) p += h.against.get(pairKey(x.id, y.id)) || 0
  return p
}
...
export function createNextMatch(players, matches, mode) {
  ...
  if (!sizeA || !sizeB) throw new Error('Ungültiger Spielmodus.')
  if (active.length < needed) throw new Error(`Für ${mode} brauchst du ${needed} aktive Spieler. Aktuell: ${active.length}.`)

  const pool = shuffle(active)
    .sort((a, b) => gamesPlayed(a.id, matches) - gamesPlayed(b.id, matches) || strength(b) - strength(a))
    .slice(0, needed)
  ...
  for (let i = 0; i < 2500; i++) {
    ...
    const teamA = c.slice(0, sizeA)
    const teamB = c.slice(sizeA, needed)
    const gameCounts = [...teamA, ...teamB].map((p) => gamesPlayed(p.id, matches))
    const score =
      Math.abs(teamSum(teamA) - teamSum(teamB)) * 100 +
      (Math.max(...gameCounts) - Math.min(...gameCounts)) * 80 +
      (teamPenalty(teamA, h) + teamPenalty(teamB, h)) * 30 +
      opponentPenalty(teamA, teamB, h) * 8 +
      Math.random()
  ...
  return {
    mode,
    team_a: best.teamA.map((p) => p.id),
    team_b: best.teamB.map((p) => p.id),
    score_a: null,
    score_b: null,
  }
}
...
export function calculateForm(players, matches) {
  ...
  for (const m of matches) {
    ...
    const winners = a > b ? m.team_a : m.team_b
    const losers = a > b ? m.team_b : m.team_a
